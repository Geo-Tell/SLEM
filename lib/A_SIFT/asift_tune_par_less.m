...
function [f, d] = asift_tune_par_less(I, V, varargin)
...
parfor(i=1:size(V,1))
     ...
     theta=V(i,2)*180/pi;
     if i==1        
        [ft, dt]=vl_sift(I,'PeakThresh',5);
     else 
        I_t = asift_simulate_image(I, t, theta);
        [ft, dt]=vl_sift(I_t,'PeakThresh',5);
     end;
     BorderFact = 6*sqrt(2.);
...
if N == 1       % compute ASIFT default samplings
    ...
    if strcmp(varargin{1}, 'frames')
        ...
        for i = 1:size(sim_samples, 2)
            ...
            [ft, dt] = vl_sift(I_tr, 'frames', f_pick,'PeakThresh',5);
